...
public class UnsortedTableMap<K, V> extends AbstractMap<K, V> {
  ...
  /**
   * Adds the entry to the map only if the key exists with a null value.
   * @param key the key with which the specified value is to be associated
   * @param value value to be associated with the specified key
   * @return the existing value if key is found with a non-null value, or null if added
   */
  public V putOnlyIfNull(K key, V value) {
    int j = findIndex(key);
    if (j == -1) {
      table.add(new MapEntry<>(key, value)); // add new entry if key doesn't exist
      return null;
    } else if (table.get(j).getValue() == null) {
      table.get(j).setValue(value); // set value if key exists with null value
      return null;
    } else {
      return table.get(j).getValue(); // return existing value if not null
    }
  }
