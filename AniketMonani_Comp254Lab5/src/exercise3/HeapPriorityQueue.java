...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
   ...
   * This ensures that the heap invariant is maintained after an insertion.
   */
  protected void upheap(int j) {
    // Base case: If j is the root, no action is needed
    if (j > 0) { // If j is not the root
      int p = parent(j); // Calculate the parent index of j
      // If the current entry at j is less than the parent entry
      if (compare(heap.get(j), heap.get(p)) < 0) {
        swap(j, p); // Swap the current entry with the parent entry
        upheap(p); // Recur on the parent index p to continue restoring the heap property
      }
    }
  }
  ...
  public static void main(String[] args)
  {
	  ...
	  // list all entries and print them
