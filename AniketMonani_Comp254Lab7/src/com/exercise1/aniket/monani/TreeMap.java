...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  protected void rebalanceAccess(Position<Entry<K,V>> p) {
	    // This method is intended to be overridden in subclasses if necessary.
	    // It is typically used to rebalance the tree after access operations,
	    // such as searching for an element. In this base class, the method does nothing.
	}

	// The remainder of the class is provided for debugging purposes only.

	/** 
	 * Prints a textual representation of the tree structure.
	 * This method is useful for visualizing the tree and understanding its structure.
	 * It recursively traverses the tree and prints each node along with its depth.
	 */
	protected void dump() {
	    // Start the recursive dumping process from the root of the tree.
	    dumpRecurse(root(), 0);
	}

	/** 
	 * Recursively prints the tree structure for debugging purposes.
	 * 
	 * @param p The current position (node) in the tree.
	 * @param depth The depth of the current node, used to format the output.
	 */
	private void dumpRecurse(Position<Entry<K,V>> p, int depth) {
	    // Create an indentation based on the depth of the node in the tree.
	    String indent = (depth == 0 ? "" : String.format("%" + (2*depth) + "s", ""));
	    
	    // Check if the current position is an external node (a leaf).
	    if (isExternal(p)) {
	        ...
	        // Recursively print the left subtree, increasing the depth.
	        dumpRecurse(left(p), depth+1);
	        
	        // Recursively print the right subtree, increasing the depth.
  ...
  private Position<Entry<K,V>> treeSearch(Position<Entry<K,V>> p, K key) {
	...
	public static void main(String[] args) {
	    // Create a new Binary Search Tree using TreeMap.
	    TreeMap<Integer,String> map = new TreeMap<Integer,String>();
	    
	    // Insert key-value pairs into the tree.
	    map.put(44, "A");  // Insert node with key 44 and value "A"
	    map.put(17, "B");  // Insert node with key 17 and value "B"
	    map.put(88, "C");  // Insert node with key 88 and value "C"
	    map.put(8, "D");   // Insert node with key 8 and value "D"
	    map.put(32, "E");  // Insert node with key 32 and value "E"
	    map.put(65, "F");  // Insert node with key 65 and value "F"
	    map.put(97, "G");  // Insert node with key 97 and value "G"
	    map.put(28, "H");  // Insert node with key 28 and value "H"
	    map.put(54, "I");  // Insert node with key 54 and value "I"
	    map.put(82, "J");  // Insert node with key 82 and value "J"
	    map.put(93, "K");  // Insert node with key 93 and value "K"
	    map.put(21, "L");  // Insert node with key 21 and value "L"
	    map.put(29, "M");  // Insert node with key 29 and value "M"
	    map.put(76, "N");  // Insert node with key 76 and value "N"
	    map.put(80, "O");  // Insert node with key 80 and value "O"

	    // Print elements in increasing order by their keys using in-order traversal.
	    System.out.println("Elements in the tree in order:");
	    for (Position<Entry<Integer,String>> p : map.tree.inorder()) {
	        System.out.println(p.getElement());
	    }

	    // Searching for a specific value associated with a key in the tree.
	    String value = map.get(21);  // Search for the value with key 28.
	    System.out.println("Value for key 21 = " + value);
	    
	    // Attempt to search for a value with a key that does not exist.
	    value = map.get(55); // This will return null as key 55 is not in the tree.
	    System.out.println("Value for key 55 = " + value);
	}
}
